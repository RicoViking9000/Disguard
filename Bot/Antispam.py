...
class Antispam(commands.Cog):
    ...
    async def checkTimedEvents(self):
        if self.checkTimedEvents.current_loop == 0:
                ...
                for e in events:
                    if discord.utils.utcnow() > e.get('expires'):
                        try:
                            if e.get('type') == 'ban':
                                try:
                                    await g.unban(await self.bot.fetch_user(e.get('target')), reason=f'{e.get("flavor")} Ban duration expired')
                                except discord.Forbidden as error:
                                    print(f'Timed ban error: {error.text}')
                                ...
                                if member:
                                    try:
                                        ...
                                        for k, v in e.get('permissionsTaken').items():
                                            await g.get_channel(int(k)).set_permissions(
                                                member,
                                                overwrite=discord.PermissionOverwrite.from_pair(discord.Permissions(v[0]), discord.Permissions(v[1])),
                                            )
                                    except discord.Forbidden as error:
                                        try:
                                            await self.bot.get_channel((await utility.get_server(g))['cyberlog']['defaultChannel']).send(
                                                f'Unable to unmute {member} because {error.text}'
                                            )
                                        except Exception:
                                            pass
    ...
    async def on_message(self, message: discord.Message):
        ...
        try:
            ...
            return
        if not spam or not (spam.get('enabled') or spam.get('attachments')[-1]):
            return  # return if antispam isn't enabled
    ...
    async def filterAntispam(self, message: discord.Message, spam):
        ...
        if any(spam.get('attachments')[:-1]) and len(message.attachments) > 0:  # Check for attachments
            ...
            for a in message.attachments:
                ...
                if t[1] and ext in audioExt + videoExt + staticExt + ['gif']:
                    aFlag = 'Media attachment'
                if t[3] and (ext in staticExt or ext == 'gif'):
                    aFlag = 'Image attachment'
                if t[7] and ext == 'gif':
                    aFlag = 'Animated image attachment'  # gif filtering
                if t[6] and ext in staticExt:
                    aFlag = 'Static image attachment'  # static image filtering
                if t[5] and ext in videoExt:
                    aFlag = 'Video attachment'  # video file filtering
                if t[4] and ext in audioExt:
                    aFlag = 'Audio attachment'
                if t[2] and ext not in audioExt + videoExt + staticExt + ['gif', 'txt']:
                    aFlag = 'Uncommon attachment'
            if aFlag:
                if t[-1]:
                    flag = True
                    reason.append(
                        f'Sending a .{ext} attachment ({aFlag})\n\nServer antispam policy blocks the following attachment types: {", ".join([descriptions[d] for d in range(len(t[:-1])) if t[d]])}'
                    )
                    ...
                    if not p.administrator or p.manage_guild or message.author.id == message.guild.owner.id:
                        ...
                        if await Cyberlog.logEnabled(message.guild, 'message') and (await utility.get_server(message.guild)).get('cyberlog').get(
                            'image'
                        ):
                            await asyncio.sleep(2)  # Wait two seconds if image logging is enabled
                        try:
                        ...
                        return await message.channel.send(
                            embed=discord.Embed(
                                description=f'Please avoid sending `{aFlag}`s in this server', color=await utility.color_theme(message.guild)
                            ),
                            delete_after=15,
                        )
        if not (any([spam.get('attachments')[:-1], spam.get('enabled')])):
            return
        cRE = await CheckRoleExclusions(message.author)
        if spam.get('exclusionMode') == 0:
            if not (message.channel.id in spam.get('channelExclusions') and cRE or message.author.id in spam.get('memberExclusions')):
                return
        else:
            if message.channel.id in spam.get('channelExclusions') or message.author.id in spam.get('memberExclusions') or cRE:
                return
        try:
        ...
        if message.content is not None and len(message.content) > 0:
            lastMessages.append(
                vars(ParodyMessage(message.content, message.created_at))
            )  # Adds a ParodyMessage object (simplified discord.Message; two variables)
        if (
            message.channel.id not in await GetChannelExclusions(message.guild)
            and not cRE
            and message.author.id not in await GetMemberExclusions(message.guild)
        ):
        ...
        if 0 not in spam.get('quickMessages') and len(quickMessages) > 0:
            ...
            if (timeLast - timeOne).seconds < spam.get('quickMessages')[1] and len(quickMessages) >= spam.get('quickMessages')[0]:
                ...
                reason.append(
                    ...
                    + str(message.author)
                    + ' sent '
                    + str(len(quickMessages))
                    + ' messages in '
                    + str((timeLast - timeOne).seconds)
                    + ' seconds'
                )
        ...
        if spam.get('mentions') != 0:
            if len(message.mentions) >= spam.get('mentions'):
                flag = True
                reason.append(
                    'Tagging too many members: '
                    + message.content
                    + '\n\n('
                    + str(len(message.mentions))
                    + ' mentions; '
                    + str(spam.get('mentions'))
                    + ' tolerated)'
                )
        ...
        if spam.get('roleTags') != 0:
            if len(message.role_mentions) >= spam.get('roleTags'):
                flag = True
                reason.append(
                    'Tagging too many roles: '
                    + message.content
                    + '\n\n('
                    + str(len(message.role_mentions))
                    + ' mentions; '
                    + str(spam.get('roleTags'))
                    + ' tolerated)'
                )
            ...
            if len(message.content) > 0:
                if changes / (len(message.content) - spaces) >= spam.get('caps') / 100.0:
                    ...
                    reason.append(
                        ...
                        + parsed
                        + '\n\n('
                        + str(round(changes / (len(message.content) - spaces) * 100))
                        + '% capital letters detected; '
                        + str(spam.get('caps'))
                        + '% tolerated)'
                    )
        ...
        if spam.get('profanityEnabled'):
            try:
                    ...
                    if filtered != message.content.lower():
                        ...
                        if censorCount / (len(filtered) - spaces) >= spam.get('profanityTolerance'):
                            ...
                            reason.append(
                                ...
                                + parsed
                                + '\n\nMessage is '
                                + str(round(censorCount / (len(filtered) - spaces) * 100))
                                + '% profanity; '
                                + str(spam.get('profanityTolerance') * 100)
                                + '% tolerated'
                            )
                ...
                pass
        if not flag:
            if len(self.antispamProcessTimes) > 5000:
                self.antispamProcessTimes.pop(0)
        ...
        if person.get('warnings') >= 1:
            ...
            if spam.get('action') in [1, 4]:
                if spam['action'] == 4:
                    role = message.guild.get_role(spam.get('customRole', 0))
                successful = await self.bot.get_cog('Moderation').muteMembers(
                    [message.author],
                    message.guild.me,
                    duration=spam.get('muteTime', 0),
                    reason='[Antispam: Automute]',
                    waitToUnmute=False,
                    muteRole=role,
                )
        ...
        shorter = discord.Embed(
            title=message.author.name + ' got busted',
            description='Hey '
            + message.author.mention
            + ', looks like you got flagged for spam for **'
            + str(len(short))
            + '** reason(s). The reason(s) is/are below, but to see these reasons in detail, please contact a moderator of '
            + message.guild.name
            + '.\n\n',
        ...
        if person.get('warnings') >= 0 and warned:
            ...
            if spam.get('action') == 4:
                shorter.add_field(
                    name='Your consequence',
                    value=message.guild.get_role(spam.get('customRole')).name + ' role for ' + str(spam.get('muteTime')) + ' seconds',
                )
        ...
        if spam.get('whisper') or not message.channel.permissions_for(message.author).read_messages:
            if not message.channel.permissions_for(message.author).read_messages:
                flavorText = f'The automute has temporarily prevented you from accessing the channel {message.channel.name}, so you would not have known you were muted otherwise. You will receive a DM update when your mute time is up.'
            else:
                flavorText = f'{message.guild.name} has set their antispam notice to DM members upon being flagged.'
            try:
        ...
        if None not in spam.get('log'):
            longer = discord.Embed(
                ...
                description=message.author.mention + ' was flagged for **' + str(len(short)) + ' reasons**, details below',
                timestamp=discord.utils.utcnow(),
                color=orange[theme],
            )
            ...
            if warned:
                longer.add_field(
                    name="Member's consequence", value='Warning (' + str(person.get('warnings')) + ' / ' + str(spam.get('warn')) + ' left)'
                )
            else:
                if spam.get('action') == 0:
        ...
        if len(self.fullAntispamProcessTimes) > 100:
            self.fullAntispamProcessTimes.pop(0)
        ...
        if not message.channel.permissions_for(message.author).read_messages:
            try:
                await message.author.send(f'You are now unmuted in {message.guild.name}')
            except discord.Forbidden:
                pass
        try:
            await self.bot.get_cog('Moderation').unmuteMembers([message.author], message.guild.me, {})
            lcEmbed = discord.Embed(
                title='Mute time is up for ' + message.author.name,
                description='Successfully removed role **' + role.name + '** from __' + message.author.mention + '__',
                timestamp=discord.utils.utcnow(),
                color=orange[theme],
            )
        except discord.Forbidden:
            lcEmbed = discord.Embed(
                title='Mute time is up for ' + message.author.name,
                description='Unable to remove role **' + role.name + '** from __' + message.author.mention + '__',
                timestamp=discord.utils.utcnow(),
                color=orange[theme],
            )
        if spam.get('log'):
    ...
    async def agekick(self, ctx: commands.Context):
        """
        View this server's configruation for the agekick module
        """
        newline = '\n'
        if not utility.ManageServer(ctx.author):
            return await ctx.send('You need manage server, administrator, or server owner permissions to use this')
        ...
        e = discord.Embed(
            title=f'Age Kick Configuration: {ctx.guild.name}',
            description=f"""**{"WHITELIST ENTRIES":–^50}**\n{newline.join([f'•{(await self.bot.fetch_user(w)).name} ({w})' for w in wl]) if wl is not None and len(wl) > 0 else '(Whitelist is empty)'}\n\n**{"RECIPIENT DM MESSAGE":–^50}**\n{config.get("ageKickDM")}""",
            color=orange[theme],
            timestamp=discord.utils.utcnow(),
        )
        e.add_field(
            name='Kick Accounts', value=f'Under {config.get("ageKick") / 86400} days old' if config.get('ageKick') is not None else 'Not enabled'
        )
    ...
    async def whitelist_user(self, ctx: commands.Context, user_id: str):
        """
        Prevents a user from being kicked by the agekick module if their account is too young
        ----------
        Parameters:
        user_id: str
        ...
        if user.id not in wl:
            ...
            e = discord.Embed(
                title=f'Added {user.display_name} to the age_kick whitelist',
            ...
            await ctx.send(f'{user.display_name} ({user.name}) is already in the whitelist')

    @whitelist_user.autocomplete('user_id')
    async def _whitelist_user_autocomplete(self, interaction: discord.Interaction, argument: str):
        try:
            user = await self.bot.fetch_user(int(argument))
        except:
            return []
        return [app_commands.Choice(name=f'{user.display_name} ({user})', value=argument)]

    @agekick.command()
    @commands.guild_only()
    async def remove_from_whitelist(self, ctx: commands.Context, user_id: str):
        """
        Removes a user from the agekick whitelist
        ----------
        Parameters:
        user_id: str
        ...
        if user.id in wl:
            ...
            e = discord.Embed(
                title=f'Removed {user.display_name} from the age_kick whitelist',
                description=f"""{self.emojis['member']}: {user}\n{self.emojis['id']}: {user.id}""",
                color=orange[await utility.color_theme(ctx.guild)],
            )
            await ctx.send(embed=e)
        else:
            await ctx.send(f'{user.name} is not in the whitelist')

    @remove_from_whitelist.autocomplete('user_id')
    async def _remove_from_whitelist_autocomplete(self, interaction: discord.Interaction, argument: str):
        whitelist = (await utility.get_server(interaction.guild)).get('antispam', {}).get('ageKickWhitelist', [])
        fetched_users = await asyncio.gather(*[self.bot.fetch_user(int(user)) for user in whitelist])
        return [
            app_commands.Choice(name=f'{user.display_name} ({user})', value=argument)
            for user in fetched_users
            if argument in str(user.id) or argument in user.name
        ]
    ...
    async def clear_whitelist(self, ctx: commands.Context):
        ...
        e = discord.Embed(
            title='Cleared the agekick whitelist', description=f'{len(wl)} users were removed', color=orange[await utility.color_theme(ctx.guild)]
        )
        await ctx.send(embed=e)

    @agekick.command()
    @commands.guild_only()
    async def set_agekick(self, ctx: commands.Context, account_age: str):
        """
        Users will be kicked from the server upon joining if their account age is under this value
        ----------
        Parameters:
        age: str
            Kick accounts younger than this value
        """
        value_seconds, int_arg, unit = utility.ParseDuration(account_age)
        config = (await utility.get_server(ctx.guild)).get('antispam')
        await database.SetAgeKick(ctx.guild, value_seconds)
    ...
    @set_agekick.autocomplete('account_age')
    async def _set_age_kick_autocomplete(self, interaction: discord.Interaction, argument: str):
        if argument:
    ...
    async def set_dm_message(self, ctx: commands.Context, message: str):
        ...
        Parameters:
        message: str
            The message to send to users who are kicked for having an account that is too young
        """
    ...
    async def set_warnings(self, ctx: commands.Context, new_warnings: int = 0, member: typing.Optional[discord.Member] = None):
        ...
        """
        embed = discord.Embed(
            title='Set Member Warnings', description=f'{self.loading}Updating member data...', color=yellow[await utility.color_theme(ctx.guild)]
        )
        if not member:
        ...
        if new_warnings == 0:
            try:
                new_warnings = server_data['antispam']['warn']
            except KeyError:
                new_warnings = 3
        oldWarnings = {}
        for member in members:
            if server_data['members'].get(str(member.id)):
                oldWarnings[member.id] = server_data['members'].get(str(member.id), {}).get('warnings', 3)
        ...
        if len(configured) == 0:
            ...
            for identification in configured:
                embed.add_field(
                    name=ctx.guild.get_member(identification), value=f'> {oldWarnings[identification]} → **{new_warnings}** warnings', inline=False
                )
                embed.description = ''
        else:
            embed.description = f'Updated warnings for {len(configured)} members\n> Set to {new_warnings} warnings'
    ...
    async def warnings(self, ctx: commands.Context):
        ...
        if warningCount:
            await ctx.send(
                f'{ctx.author.mention}, you have {warningCount} warning{"s" if warningCount != 1 else ""} in my antispam system',
                allowed_mentions=mentions,
            )
        else:
            await ctx.send(f'{ctx.author.mention}, I was unable to retrieve your warning count')
