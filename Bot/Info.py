...
class Info(commands.Cog):
    ...
    async def info(
    ...
    ):  # queue system: message, embed, every 3 secs, check if embed is different, edit message to new embed
        arg = search.lower()
        ...
        if len(arg) > 0:
            members, roles, channels, emojis = (
                utility.FindMembers(ctx.guild, arg),
                utility.FindRoles(ctx.guild, arg),
                utility.FindChannels(ctx.guild, arg),
                utility.FindEmojis(ctx.guild, arg),
            )
        ...
        for c in channels:
            types = {
                discord.TextChannel: self.emojis['textChannel'],
                discord.VoiceChannel: self.emojis['voiceChannel'],
                discord.CategoryChannel: self.emojis['folder'],
            }
        ...
        if any(s == arg for s in ['him', 'her', 'them', 'it', '^']):
            ...
            def pred(m):
                return m.author != ctx.guild.me and m.author != ctx.author

            author = sorted([message async for message in ctx.channel.history(limit=100).filter(pred)], key=lambda m: m.created_at, reverse=True)[
                -1
            ].author
        ...
        if len(arg) == 0:
            ...
            def birthdayCheck(r, u):
                return u == ctx.author and r.message.id == message.id and str(r) == '🍰'
        ...
        members, roles, channels, inv, emojis = tuple(
            await asyncio.gather(
                *[
                    utility.FindMoreMembers(ctx.guild.members, arg),
                    utility.FindMoreRoles(ctx.guild, arg),
                    utility.FindMoreChannels(ctx.guild, arg),
                    utility.FindMoreInvites(ctx.guild, arg),
                    utility.FindMoreEmojis(ctx.guild, arg),
                ]
            )
        )
        counter = 0
        while counter < 2:
            ...
            for m in members:
                every.append(
                    InfoResult(
                        m.get('member'), '👤{} - {} ({}% match)'.format(m.get('member').name, m.get('check')[0], m.get('check')[1]), m.get('check')[1]
                    )
                )
            for r in roles:
                every.append(
                    InfoResult(
                        r.get('role'), '🚩{} - {} ({}% match)'.format(r.get('role').name, r.get('check')[0], r.get('check')[1]), r.get('check')[1]
                    )
                )
            for c in channels:
                every.append(
                    InfoResult(
                        c.get('channel'),
                        '{}{} - {} ({}% match)'.format(
                            types.get(type(c.get('channel'))), c.get('channel').name, c.get('check')[0], c.get('check')[1]
                        ),
                        c.get('check')[1],
                    )
                )
            for i in inv:
                every.append(
                    InfoResult(
                        i.get('invite'),
                        '💌discord.gg/{} - {} ({}% match)'.format(
                            i.get('invite').code.replace(arg, '**{}**'.format(arg)), i.get('check')[0], i.get('check')[1]
                        ),
                        i.get('check')[1],
                    )
                )
            for e in emojis:
                every.append(
                    InfoResult(
                        e.get('emoji'),
                        '{}{} - {} ({}% match)'.format(e.get('emoji'), e.get('emoji').name, e.get('check')[0], e.get('check')[1]),
                        e.get('check')[1],
                    )
                )
            ...
            if 'server' in arg or 'guild' in arg or arg in ctx.guild.name.lower() or ctx.guild.name.lower() in arg:
                every.append(
                    InfoResult(
                        (await self.ServerInfo(ctx.guild, logs, bans, webhooks, invites)), 'ℹServer information', utility.compareMatch('server', arg)
                    )
                )
            if 'roles' in arg:
                every.append(
                    InfoResult((await self.RoleListInfo(ctx.guild.roles, logs)), 'ℹRole list information', utility.compareMatch('roles', arg))
                )
            if any(s in arg for s in ['members', 'people', 'users', 'bots', 'humans']):
                every.append(
                    InfoResult((await self.MemberListInfo(ctx.guild.members)), 'ℹMember list information', utility.compareMatch('members', arg))
                )
            if 'channels' in arg:
                every.append(
                    InfoResult(
                        (await self.ChannelListInfo(ctx.guild.channels, logs)), 'ℹChannel list information', utility.compareMatch('channels', arg)
                    )
                )
            if 'emoji' in arg or 'emotes' in arg:
                every.append(InfoResult((await self.EmojiListInfo(ctx.guild.emojis, logs)), 'ℹEmoji information', utility.compareMatch('emoji', arg)))
            if 'invites' in arg:
                every.append(
                    InfoResult((await self.InvitesListInfo(invites, logs, ctx.guild)), 'ℹInvites information', utility.compareMatch('invites', arg))
                )
            if 'bans' in arg:
                every.append(InfoResult((await self.BansListInfo(bans, logs, ctx.guild)), 'ℹBans information', utility.compareMatch('bans', arg)))
            if any(s in arg for s in ['dev', 'owner', 'master', 'creator', 'author', 'disguard', 'bot', 'you']):
                every.append(
                    InfoResult(
                        (await self.BotInfo(await self.bot.application_info(), ctx.guild)),
                        '{}Information about me'.format(self.bot.get_emoji(569191704523964437)),
                        utility.compareMatch('disguard', arg),
                    )
                )
            ...
            md2 = []
            used = md.format(
                1 if len(every) >= 1 else 0,
                20 if len(every) >= 20 else len(every),
                len(every),
                arg,
                ' (Arrows to scroll)' if len(every) >= 20 else '',
            )
            main.description = used
            for result in range(len(every)):
                md2.append('\n{}: {}'.format(result + 1, every[result].mainKey))
            main.description += ''.join(md2[:20])
            if len(main.description) > 2048:
                main.description = main.description[:2048]
            if len(every) == 0 and indiv is None:
            ...
            if counter == 0:
                try:
                    logs = [log async for log in ctx.guild.audit_logs(limit=None)]
                except:
                    logs = False  # signify failure to the end user
                try:
        ...
        while not self.bot.is_closed():
            ...
            if past or (message.embeds[0].author.name and '⭐' in message.embeds[0].author.name):
                ...
                def checkBday(r, u):
                    return u == desired and not u.bot and r.message.id == message.id and str(r) == '🍰'

                def checkBack(r, u):
                    return u == ctx.author and r.message.id == message.id and str(r) == '⬅'

                if 'member details' in message.embeds[0].title.lower() and desired:
                    await message.add_reaction('🍰')
            ...
            def check(m):
                try:
                    return m.author == ctx.author and int(m.content) <= len(every)
                except:
                    return False
            ...
            def reacCheck(r, u):
                return str(r) in ['◀', '▶'] and u == ctx.author
            ...
            while not past:
                ...
                if type(stuff) is tuple:
                    ...
                    if str(stuff[0]) == '▶':
                        coords = coords[0] + 20, coords[1] + 20
                    if coords[0] < 0:
                        coords = 0, 20 if len(every) > 20 else len(every)
                    if coords[1] > len(every):
                        coords = coords[0], len(every)
                    used = md.format(coords[0], coords[1], len(every), arg, ' (Arrows to scroll)' if len(every) >= 20 else '') + ''.join(
                        md2[coords[0] - 1 : coords[1]]
                    )
                    main.description = used
                    await message.edit(embed=main)
                else:
                    ...
                    if type(every[int(stuff.content) - 1].obj) is discord.Member:
                        for i, f in enumerate(message.embeds[0].fields):
                            if '📜Messages' == f.name:
                                ...
                                break
                    await message.add_reaction('⬅')
                    if 'member details' in message.embeds[0].title.lower():
                        await message.add_reaction('🍰')
    ...
    async def ServerInfo(self, s: discord.Guild, logs, bans, hooks, invites):
        ...
        embed = discord.Embed(
            title=s.name,
            description='' if s.description is None else '**Server description:** {}\n\n'.format(s.description),
            timestamp=discord.utils.utcnow(),
            color=yellow[await utility.color_theme(s)],
        )
        mfa = {0: 'No', 1: 'Yes'}
        veri = {
            'none': 'None',
            'low': 'Email',
            'medium': 'Email, account age > 5 mins',
            'high': 'Email, account 5 mins old, server member for 10 mins',
            'extreme': 'Phone number',
        }
        perks0 = ['None yet']
        perks1 = [
            '100 emoji limit, 128kbps bitrate',
            'animated server icon, custom server invite background',
        ]  # First half doesn't get added to string for later levels
        ...
        if s.premium_tier == 3:
            perks = [perks3[0], perks3[1], perks2[1], perks1[1]]
        elif s.premium_tier == 2:
            perks = [perks2[0], perks2[1], perks1[1]]
        elif s.premium_tier == 1:
            perks = perks1
        else:
            perks = perks0
        messages = 0
        ...
        onlineGeneral = 'Online: {} / {} ({}%)'.format(
            len([m for m in s.members if m.status != discord.Status.offline]),
            len(s.members),
            round(len([m for m in s.members if m.status != discord.Status.offline]) / len(s.members) * 100),
        )
        offlineGeneral = 'Offline: {} / {} ({}%)'.format(
            len([m for m in s.members if m.status == discord.Status.offline]),
            len(s.members),
            round(len([m for m in s.members if m.status == discord.Status.offline]) / len(s.members) * 100),
        )
        ...
        embed.description += '\n\n**Member count:** {}{}\n{}'.format(
            len(s.members),
            '' if s.max_members is None else '/{}'.format(s.max_members) if s.max_members - len(s.members) < 500 else '',
            '\n'.join([onlineGeneral, offlineGeneral, online, idle, dnd, offline]),
        )
        embed.description += '\n\n**Features:** {}'.format(', '.join(s.features) if len(s.features) > 0 else 'None')
        embed.description += '\n\n**Nitro boosters:** {}/{}, **perks:** {}'.format(
            s.premium_subscription_count, perkDict.get(s.premium_tier), ', '.join(perks)
        )
        # embed.set_thumbnail(url=s.icon_url)
        embed.add_field(
            name='Created', value=f'{utility.DisguardIntermediateTimestamp(created)} ({utility.DisguardRelativeTimestamp(created)})', inline=False
        )
        embed.add_field(name='AFK Timeout', value='{}s --> {}'.format(s.afk_timeout, s.afk_channel))
        if s.max_presences is not None:
            embed.add_field(name='Max Presences', value='{}'.format(s.max_presences))
        ...
        try:
            embed.add_field(name='Locale', value=s.preferred_locale)
        except:
            pass
        embed.add_field(
            name='Audit logs',
            value=self.loading if logs is None else '🔒Unable to obtain audit logs' if logs is False else len(logs) if logs else 'N/A',
        )
        if s.system_channel is not None:
            embed.add_field(
                name='System channel', value='{}: {}'.format(s.system_channel.mention, ', '.join([k[0] for k in (iter(s.system_channel_flags))]))
            )
        embed.add_field(name='Role count', value=len(s.roles) - 1)
        embed.add_field(name='Owner', value=s.owner.mention)
        embed.add_field(
            name='Banned members',
            value=self.loading if bans is None else '🔒Unable to obtain server bans' if bans is False else len(bans) if bans else 'N/A',
        )
        embed.add_field(
            name='Webhooks',
            value=self.loading if hooks is None else '🔒Unable to obtain webhooks' if hooks is False else len(hooks) if hooks else 'N/A',
        )
        embed.add_field(
            name='Invites',
            value=self.loading if invites is None else '🔒Unable to obtain invites' if invites is False else len(invites) if invites else 'N/A',
        )
    ...
    async def ChannelInfo(self, channel: discord.abc.GuildChannel, invites, pins, logs):
        ...
        details = discord.Embed(
            title=f'{utility.channelEmoji(self, channel)}{channel.name}',
            description='',
            color=yellow[await utility.color_theme(channel.guild)],
            timestamp=discord.utils.utcnow(),
        )
        details.set_footer(text='Channel ID: {}'.format(channel.id))
        if type(channel) is discord.TextChannel:
            details.description += channel.mention
        ...
        for k, v in channel.overwrites.items():
            perms.update({k: dict(iter(v))})
        for k, v in perms.items():
            for kk, vv in v.items():
                if vv is not None:
                    try:
                        formatted.get(kk).update({k: vv})
                    except:
                        formatted.update({kk: {k: vv}})
        ...
        details.add_field(
            name='Last updated',
            value=f'{utility.DisguardIntermediateTimestamp(updated)} ({utility.DisguardRelativeTimestamp(updated)})'
            if logs != None
            else self.loading
            if logs is None
            else '🔒Unable to obtain audit logs'
            if not logs
            else 'N/A',
        )
        inviteCount = []
        if invites:
            for inv in iter(invites):
                inviteCount.append(inv.inviter)
        details.add_field(
            name='Invites to here',
            value=self.loading
            if invites is None
            else '🔒Unable to retrieve invites'
            if invites is False
            else 'None'
            if len(inviteCount) == 0 or not logs
            else ', '.join(['{} by {}'.format(a[1], a[0].name) for a in iter(collections.Counter(inviteCount).most_common())]),
        )
        if type(channel) is discord.TextChannel:
            details.add_field(
                name='Topic',
                value='{}{}'.format(
                    '<No topic>' if channel.topic is None or len(channel.topic) < 1 else channel.topic[:100],
                    '' if channel.topic is None or len(channel.topic) <= 100 else '...',
                ),
                inline=False,
            )
        ...
        if type(channel) is discord.VoiceChannel:
            ...
            details.add_field(
                name='Members currently in here',
                value='None' if len(channel.members) == 0 else ', '.join([member.mention for member in channel.members]),
            )
    ...
    async def RoleInfo(self, r: discord.Role, logs):
        ...
        if updated is None:
            updated = created
        embed = discord.Embed(
            title='🚩Role: {}'.format(r.name),
            description='**Permissions:** {}'.format(
                'Administrator'
                if r.permissions.administrator
                else ' • '.join([utility.permissionKeys.get(p[0], f'Unknown Permission ({p[0]})') for p in iter(r.permissions) if p[1]])
            ),
            timestamp=discord.utils.utcnow(),
            color=r.color,
        )
    ...
    async def MemberInfo(self, m: discord.Member, *, addThumbnail=True, calculatePosts=True):
        ...
        activeTimestamp = mA.get(
            'timestamp'
        )  # .replace(tzinfo=datetime.timezone.utc)# + datetime.timedelta(hours=timeZone(m.guild) + 4) #The timestamp value when a member was last active, with adjustments for timezones
        onlineTimestamp = await Cyberlog.lastOnline(
            m
        )  # .replace(tzinfo=datetime.timezone.utc)# + datetime.timedelta(hours=timeZone(m.guild) + 4) #The timestamp value when a member was last online, with adjustments for timezones
        ...
        hours, minutes, seconds = (
            activeDelta.seconds // 3600,
            (activeDelta.seconds // 60) % 60,
            activeDelta.seconds - (activeDelta.seconds // 3600) * 3600 - ((activeDelta.seconds // 60) % 60) * 60,
        )
        activeTimes = [seconds, minutes, hours, activeDelta.days]  # List of self explanatory values
        hours, minutes, seconds = (
            onlineDelta.seconds // 3600,
            (onlineDelta.seconds // 60) % 60,
            onlineDelta.seconds - (onlineDelta.seconds // 3600) * 3600 - ((onlineDelta.seconds // 60) % 60) * 60,
        )
        ...
        for i in range(len(activeTimes)):
            if activeTimes[i] != 0:
                activeDisplay.append('{}{}'.format(activeTimes[i], units[i][0]))
            if onlineTimes[i] != 0:
                onlineDisplay.append('{}{}'.format(onlineTimes[i], units[i][0]))
        if len(activeDisplay) == 0:
            activeDisplay = ['0s']
        activities = {
            discord.Status.online: self.emojis['online'],
            discord.Status.idle: self.emojis['idle'],
            discord.Status.dnd: self.emojis['dnd'],
            discord.Status.offline: self.emojis['offline'],
        }
            ...
            for act in m.activities:
                try:
                    if act.type is discord.ActivityType.playing:
                        try:
                            current.append(
                                f'playing {act.name}: {act.details}{(", " + act.state) if act.state is not None else ""}{" (⭐Visible under username)" if act == m.activity else ""}'
                            )
                        except AttributeError:
                            current.append(f'playing {act.name}{" (⭐Visible under username)" if act == m.activity else ""}')
                    elif act.type is discord.ActivityType.custom:
                        current.append(
                            f'{act.emoji if act.emoji is not None else ""} {act.name if act.name is not None else ""}{" (⭐Visible under username)" if act == m.activity else ""}'
                        )
                    elif act.type is discord.ActivityType.streaming:
                        current.append(f'streaming {act.name}{" (⭐Visible under username)" if act == m.activity else ""}')
                    elif act.type is discord.ActivityType.listening and act.name == 'Spotify':
                        current.append(
                            f'Listening to Spotify{" (⭐Visible under username)" if act == m.activity else ""}\n 🎵 {act.title}\n 👤 {", ".join(act.artists)}\n 💿 {act.album}'
                        )
                    elif act.type is discord.ActivityType.watching:
                        current.append(f'watching {act.name}{" (⭐Visible under username)" if act == m.activity else ""}')
                except:
                    current.append('Error parsing activity')
            embed.description += '\n\n • {}'.format('\n • '.join(current))
        embed.description += '\n\n**Roles ({}):** {}\n\n**Permissions:** {}\n\nReact 🍰 to switch to Birthday Information view'.format(
            len(m.roles) - 1,
            ' • '.join([r.name for r in reversed(m.roles)]),
            'Administrator'
            if m.guild_permissions.administrator
            else ' • '.join([utility.permissionKeys.get(p[0], f'Unknown Permission ({p[0]})') for p in iter(m.guild_permissions) if p[1]]),
        )
        ...
        if m.voice is None:
            ...
            voice = '{}{} in {}{}'.format(
                '🔇' if m.voice.mute or m.voice.self_mute else '',
                '🤐' if m.voice.deaf or m.voice.self_deaf else '',
                'N/A' if m.voice.channel is None else m.voice.channel.name,
                ', AFK' if m.voice.afk else '',
            )
        if boosting is None:
            embed.add_field(name='Boosting server', value='Nope')
        else:
        ...
        if addThumbnail:
            embed.set_thumbnail(url=m.display_avatar.url)  # V2.0
    ...
    async def EmojiInfo(self, e: discord.Emoji, owner):
        ...
        if owner is not None:
            embed.add_field(name='Uploaded by', value='{} ({})'.format(owner.mention, owner.name))
    ...
    async def InviteInfo(self, i: discord.Invite, s):  # s: server
        embed = discord.Embed(
            title='Invite details', description=str(i), timestamp=discord.utils.utcnow(), color=yellow[await utility.color_theme(s)]
        )
    ...
    async def BotInfo(self, app: discord.AppInfo, s: discord.Guild):
        ...
        embed = discord.Embed(
            title='About Disguard',
            description='{0}{1}{0}'.format(self.bot.get_emoji(569191704523964437), app.description),
            timestamp=discord.utils.utcnow(),
            color=yellow[await utility.color_theme(s)],
        )
        ...
        embed.add_field(name='Website with information', value='[Disguard Website](https://disguard.netlify.app/)')
    ...
    async def EmojiListInfo(self, emojis, logs):
        """Prereq: len(emojis) > 0"""
        embed = discord.Embed(
            title="{}'s emojis".format(emojis[0].guild.name),
            description='Total emojis: {}'.format(len(emojis)),
            timestamp=discord.utils.utcnow(),
            color=yellow[await utility.color_theme(emojis[0].guild)],
        )
        static = [str(e) for e in emojis if not e.animated]
        animated = [str(e) for e in emojis if e.animated]
        if len(static) > 0:
            embed.add_field(name='Static emojis: {}/{}'.format(len(static), emojis[0].guild.emoji_limit), value=''.join(static)[:1023], inline=False)
        if len(animated) > 0:
            embed.add_field(
                name='Animated emojis: {}/{}'.format(len(animated), emojis[0].guild.emoji_limit), value=''.join(animated)[:1023], inline=False
            )
        if logs:
            embed.add_field(
                name='Total emojis ever created',
                value='At least {}'.format(len([l for l in logs if l.action == discord.AuditLogAction.emoji_create])),
            )
        elif logs is None:
            embed.add_field(name='Total emojis ever created', value=self.loading)
        else:
            embed.add_field(name='Total emojis ever created', value='🔒Unable to obtain audit logs')
        return embed

    async def ChannelListInfo(self, channels, logs):
        """Prereq: len(channels) > 0"""
        embed = discord.Embed(
            title="{}'s channels".format(channels[0].guild.name),
            timestamp=discord.utils.utcnow(),
            color=yellow[await utility.color_theme(channels[0].guild)],
        )
        none = ['(No category)'] if len([c for c in channels if type(c) is not discord.CategoryChannel and c.category is None]) else []
        none += [
            '|{}{}'.format(utility.channelEmoji(self, c), c.name) for c in channels if type(c) is not discord.CategoryChannel and c.category is None
        ]
        ...
        if logs:
            embed.add_field(
                name='Total channels ever created',
                value='At least {}'.format(len([l for l in logs if l.action == discord.AuditLogAction.channel_create])),
            )
        elif logs is None:
            embed.add_field(name='Total channels ever created', value=self.loading)
        else:
            embed.add_field(name='Total channels ever created', value='🔒Unable to obtain audit logs')
        return embed

    async def RoleListInfo(self, roles, logs):
        """Prereq: len(roles) > 0"""
        embed = discord.Embed(
            title="{}'s roles".format(roles[0].guild.name), timestamp=discord.utils.utcnow(), color=yellow[await utility.color_theme(roles[0].guild)]
        )
        ...
        if logs:
            embed.add_field(
                name='Total roles ever created', value='At least {}'.format(len([l for l in logs if l.action == discord.AuditLogAction.role_create]))
            )
        elif logs is None:
            embed.add_field(name='Total roles ever created', value=self.loading)
        else:
            embed.add_field(name='Total roles ever created', value='🔒Unable to obtain audit logs')
        return embed

    async def MemberListInfo(self, members):
        embed = discord.Embed(
            title="{}'s members".format(members[0].guild.name),
            description='',
            timestamp=discord.utils.utcnow(),
            color=yellow[await utility.color_theme(members[0].guild)],
        )
        posts = []
        ...
        onlineGeneral = 'Online: {} / {} ({}%)'.format(
            len([m for m in members if m.status != discord.Status.offline]),
            len(members),
            round(len([m for m in members if m.status != discord.Status.offline]) / len(members) * 100),
        )
        offlineGeneral = 'Offline: {} / {} ({}%)'.format(
            len([m for m in members if m.status == discord.Status.offline]),
            len(members),
        ...
        embed.description += '\n\n**Member count:** {}{}\n{}'.format(
            len(members),
            '' if members[0].guild.max_members is None else '/{}'.format(members[0].guild.max_members),
    ...
    async def InvitesListInfo(self, invites, logs, s: discord.Guild):
        embed = discord.Embed(
            title=f'{invites[0].guild.name if invites else "This server"}\'s invites',
            timestamp=discord.utils.utcnow(),
            color=yellow[await utility.color_theme(s)],
        )
        if invites:
            ...
            embed.description = f'Total invites: {self.loading if invites is None else "🔒Unable to obtain invites"}'
        if logs:
            embed.add_field(
                name='Total invites ever created',
                value='At least {}'.format(len([l for l in logs if l.action == discord.AuditLogAction.invite_create])),
            )
        elif logs is None:
            embed.add_field(name='Total invites ever created', value=self.loading)
        else:
            embed.add_field(name='Total invites ever created', value='🔒Unable to obtain audit logs')
        return embed
    ...
    async def BansListInfo(self, bans, logs, s):  # s=server
        ...
        embed.description = (
            f'Users currently banned: {len(bans) if bans else self.loading if bans is None else "🔒Missing ban retrieval permissions"}'
        )
        if not logs:
            if logs is None:
                embed.add_field(name='Banned previously', value=self.loading)
            else:
                embed.add_field(name='Banned previously', value='🔒Unable to obtain audit logs')
        if not bans and not logs:
            return embed
        null = embed.copy()
        array = []
        current = []
        for b in bans:
            for l in logs:
                if l.action == discord.AuditLogAction.ban and l.target == b.user:
                    ...
                    array.append(
                        '{}: Banned by {} on {} because {}'.format(
                            l.target.name, l.user.name, created.strftime('%m/%d/%Y@%H:%M'), '(No reason specified)' if b.reason is None else b.reason
                        )
                    )
        ...
        for l in logs:
            if l.action == discord.AuditLogAction.ban and l.target not in current:
                ...
                other.append(
                    '{}: Banned by {} on {} because {}'.format(
                        l.target.name, l.user.name, created.strftime('%m/%d/%Y@%H:%M'), '(No reason specified)' if l.reason is None else l.reason
                    )
                )
                current.append(b.user)
        for b in bans:
            if b.user not in current:
    ...
    async def evalInfo(self, obj, g: discord.Guild, logs):
        if type(obj) is discord.Embed:
            return obj
        if type(obj) is discord.Member:
            return await self.MemberInfo(obj, calculatePosts=False)
        if type(obj) is discord.Role:
            return await self.RoleInfo(obj, logs)
        if type(obj) in [discord.TextChannel, discord.VoiceChannel, discord.CategoryChannel]:
            try:
                invites = await obj.invites()
            except:
                invites = False
            try:
                pins = await obj.pins()
        ...
        if type(obj) is discord.Emoji:
            return await self.EmojiInfo(obj, (await obj.guild.fetch_emoji(obj.id)).user)
        if type(obj) is discord.Invite:
            return await self.InviteInfo(obj, g)
        if type(obj) is discord.PartialEmoji:
            return await self.PartialEmojiInfo(obj, g)
