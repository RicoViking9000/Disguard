...
class Birthdays(commands.GroupCog, name='birthdays', description='Birthday module commands'):
    ...
    async def dailyBirthdayAnnouncements(self):
        ...
        for userID in birthdayDict[datetime.date.today().strftime('%m%d')]:
            ...
            try:
                bday: datetime.datetime = user_data.get('birthday')
            except KeyError:
                continue  # User not in cache
            # If there's no birthday set for this user or they've disabled the birthday module, return
            if not bday or not await cyber.privacyEnabledChecker(user, 'birthdayModule', 'birthdayDay'):
                continue
            ...
            embed = discord.Embed(
                title=f'🍰 Happy {f"{age - 1}{utility.suffix(age - 1)}" if age > 1 else ""}Birthday, {user.display_name}! 🍰', color=yellow[1]
            )
            ...
            messageEmbeds = [embed] + [
                discord.Embed(title=f'✉ Birthday Message from {m["authName"]}', description=m['message'], color=yellow[1]) for m in filteredMessages
            ]
            # Add a disclaimer footer to the last embed sent
            if filteredMessages:
                messageEmbeds[-1].set_footer(
                    "The developer of Disguard and server moderators have no say in the contents of messages sent by other users. If any of these messages are inappropriate, please contact the moderator of a server you share with the user, or get in contact with Disguard's developer. You may also delete one of these messages from our DMs with `.delete <ID of message containing offensive message>`."
                )
            ...
            try:
                for page in embedsToSend:
                    await user.send(embeds=page)
    ...
    async def serverBirthdayAnnouncements(self):
        ...
        for userID in birthdayDict.get(datetime.date.today().strftime('%m%d'), []):
            user = self.bot.get_user(userID)
            user_data = await utility.get_user(user)
            if not await cyber.privacyEnabledChecker(user, 'birthdayModule', 'birthdayDay'):
                continue
            try:
                bday: datetime.datetime = user_data.get('birthday')
            except KeyError:
            ...
            for server in servers:
                ...
                channel = self.bot.get_channel(
                    server_data.get('birthday')
                )  # Doing this instead of try/except since birthday channels usually default to 0 if not set
                if started.strftime('%H:%M') == server_data.get('birthdate', datetime.datetime.min).strftime('%H:%M') or not channel:
                    continue
                # print(f'Announcing birthday for {member.display_name} to {server.name}')
                messages = (
                    [a for a in user_data.get('birthdayMessages', []) if server.id in a['servers']]
                    if await cyber.privacyEnabledChecker(user, 'birthdayModule', 'birthdayMessages')
                    else []
                )
                messageString = (
                    f"Members from this server also wrote {len(messages)} birthday messages to be delivered here on {user.display_name}'s birthday:"
                    if messages
                    else ''
                )
                if userID == 247412852925661185:
                    toSend = (
                        f"🍰🎊🍨🎈 Greetings {server.name}! It's my developer {user.mention}'s birthday!! Let's wish him a very special day! 🍰🎊🍨🎈"
                    )
                else:
                    if await cyber.privacyVisibilityChecker(user, 'birthdayModule', 'birthdayDay'):
                        toSend = f"🍰 Greetings {server.name}, it's {user.mention}'s birthday! Let's all wish them a very special day! 🍰"
                    else:
                        toSend = f"🍰 Greetings {server.name}! We have an anonymous member with a birthday today! Let's all wish them a very special day! 🍰"
                if messages:
                    toSend += f'\n{messageString}'
                    messageEmbeds = [
                        discord.Embed(title=f'✉ Birthday Message from {m["authName"]}', description=m['message'], color=yellow[1]) for m in messages
                    ]
                    embedsToSend = utility.paginate(messageEmbeds, 10)
                m = await self.bot.get_channel(channel).send(
                    toSend, embeds=embedsToSend[0]
                )  # Caveat: moderators can't delete individual embeds if inappropriate
                if len(embedsToSend) > 1:
                    for page in embedsToSend[1:]:
                        await self.bot.get_channel(channel).send(embeds=embedsToSend[page])
    ...
    async def deleteBirthdayMessages(self):
        for user in self.bot.users:
            try:
                bday: datetime.datetime = (await utility.get_user(user)).get('birthday')
            except KeyError:
                continue  # if user not found in the cache
            if bday:
                if bday.strftime('%m%d%y') == discord.utils.utcnow().strftime('%m%d%y'):
                    await database.ResetBirthdayMessages(user)
    ...
    async def verifyBirthdaysDict(self):
        ...
        for user in self.bot.users:
            try:
                ...
                try:
                    bday: datetime.datetime = await database.GetMemberBirthday(user)
                except AttributeError:
                    continue
            if not bday:
                continue
            if user.id not in birthdayList[bday.strftime('%m%d')]:
                birthdayList[bday.strftime('%m%d')].append(user.id)
    ...
    async def on_message(self, message: discord.Message):
        ...
        try:
            if server_data.get('birthdayMode') in [None, 0]:
                return  # Birthday auto detect is disabled
            if not await self.bot.get_cog('Cyberlog').privacyEnabledChecker(message.author, 'default', 'birthdayModule'):
                return  # This person disabled the birthday module
        except AttributeError:
            return
    ...
    async def birthdayMessagehandler(self, message: discord.Message, server_data: dict):
        cyber: Cyberlog.Cyberlog = self.bot.get_cog('Cyberlog')
        if not await cyber.privacyEnabledChecker(message.author, 'birthdayModule', 'birthdayDay'):
            return  # User disabled the birthday features
        ...
        if birthday and target:
            if server_data.get('birthdayMode') == 1:
                ...
                def cakeAutoVerify(r: discord.Reaction, u: discord.User):
                    return u == message.author and str(r) == '🍰' and r.message.id == message.id

                await message.add_reaction('🍰')
                await self.bot.wait_for('reaction_add', check=cakeAutoVerify)
            if birthday < adjusted:
    ...
    async def ageMessageHandler(self, message: discord.Message, server_data: dict):
        cyber: Cyberlog.Cyberlog = self.bot.get_cog('Cyberlog')
        if not await cyber.privacyEnabledChecker(message.author, 'birthdayModule', 'age'):
            return  # User disabled the age features
        ages = calculateAges(message.content)
        ages = [a for a in ages if await verifyAge(message.content, a)] if ages else []
        if not ages:
            return  # No ages detected in message
        if (await utility.get_server(message.guild)).get('birthdayMode') == 1:  # Make user add candle reaction

            def candleAutoVerify(r: discord.Reaction, u: discord.User):
                return u == message.author and str(r) == '🕯' and r.message.id == message.id
        ...
        if len(ages) > 1:
            ...
            def interactionCheck(i: discord.Interaction):
                return i.data['custom_id'] == ageView.select.custom_id and i.user == message.author and i.channel == message.channel

            try:
                await self.bot.wait_for('interaction', check=interactionCheck, timeout=300)
    ...
    async def set_birthday(
        ...
        await interaction.response.send_message(
            embed=embed,
            view=view,
            ephemeral=interaction.guild and not await cyber.privacyVisibilityChecker(interaction.user, 'birthdayModule', 'birthdayDay'),
        )
        await view.confirmation()
    ...
    async def set_age(self, interaction: discord.Interaction, new_age: app_commands.Range[int, 13, 105]):
        ...
        await interaction.response.send_message(
            embed=embed,
            view=view,
            ephemeral=interaction.guild and not await cyber.privacyVisibilityChecker(interaction.user, 'birthdayModule', 'age'),
        )
        await view.confirmation()
...
def calculateDate(message: str, adjusted: datetime.datetime):
    ...
    ref = collections.deque(
        [(a, b) for a, b in {1: 31, 2: 29 if isLeapYear() else 28, 3: 31, 4: 30, 5: 31, 6: 30, 7: 31, 8: 31, 9: 30, 10: 31, 11: 30, 12: 31}.items()]
    )
    ...
    if any(month in words for month in longMonths):
        months = longMonths
    else:
        months = shortMonths
    # Determine if the user used long day format (such as Monday) or short (such as Mon)
    if any(day in words for day in longDays):
        days = longDays
    else:
        days = shortDays
    ...
    if containsMonth or 'the' in words:
        ...
        for word in words:
            ...
            # if there's no month in the word and the word was succesfully truncated above, try getting a date from the current year and month (such as the user passing "the 6th")
                ...
                if before != word:
                    try:
                        birthday = datetime.datetime(now.year, now.month, int(word))
                    except:
                        pass
    # Check if day of the week is in message
        ...
        if birthday < adjusted and 'was' not in words:
            birthday += datetime.timedelta(
                days=7
            )  # If target is a weekday already past, jump it to next week; since that's what they mean if they didn't say 'was' in their sentence
    elif any(phrase in words for phrase in ['today', 'yesterday', 'ago', 'tomorrow']):
        if any(phrase in message for phrase in ['my birthday', 'my bday' 'mine is']) and 'today' in words:
            birthday = adjusted
        elif any(phrase in message for phrase in ['my birthday', 'my bday' 'mine was']) and 'yesterday' in words:
            ...
            for word in words:
                try:
                    ...
                    continue
                if any(w in words for w in ['day', 'days']):
                    birthday = adjusted - datetime.timedelta(days=num)
                if any(w in words for w in ['week', 'weeks']):
                    birthday = adjusted - datetime.timedelta(days=num * 7)
                if any(w in words for w in ['month', 'months']):
                    birthday = adjusted - datetime.timedelta(
                        days=sum(a[1] for a in list(ref)[-1 * num :])
                    )  # Jump back [num] months; starting from end of list because ago = back in time; need to get correct days
        ...
        for word in words:
            try:
                birthday = datetime.datetime.strptime(word, '%m/%d/%y')
            except:
                try:
                    birthday = datetime.datetime.strptime(word, '%m-%d-%y')
                except:
...
async def verifyBirthday(message: typing.Union[str, discord.Message], adjusted: datetime.datetime, birthday=None):
    ...
    # User most likely talking about someone else's birthday
        ...
        if (
            any(word in words for word in ['is', 'are'])
            and not any(word in words for word in ['my', 'mine'])
            and len(message.mentions) > 0
            and any(word in message.content.lower() for word in ['birthday', 'bday'])
        ):
...
async def verifyAge(message: str, age):
    ...
    if any(word in message for word in ['im', "i'm"]) or 'i am' in message:  # Deal with age
        if 'i am' in message:
            ...
            finder = 'im'
        try:
            number = int(message[1 + message.index(finder)])
        except:
            return False
        if abs(message.index(str(number)) - message.index(finder)) > 1:
            return False  # I'm or I am is too far from the actual number so it's irrelevant
        if number:
            try:
                tail = message[message.index(str(number)) :]  # If there is content after the number, try to deal with it
            except:
                if int(message[1 + message.index(finder)]) not in calculateAges(message):
                    return False  # If the relevant age is not the same one found in the message, return
                return False
            if len(tail) > 1:
                if 'year' not in tail[1]:
                    # Parts of speech analysis
                    if tagged[1 + message.index(str(number))][1] not in ['IN', 'CC']:
                        return False  # Part of speech after age number makes this not relevant
                else:
                    if len(tail) > 2:
                        if 'old' not in tail[2]:
                            return False
    elif 'age is' in message:
        if 'my' not in words:
            return False
        try:
            int(message[message.index('age') + 2])
        except:
            return False
    else:
        return False
...
def mutualServerMemberToMember(self: Birthdays, memberA: discord.User, memberB: discord.User):
    ...
    for g in self.bot.guilds:
        ...
        foundB = False
        for m in g.members:
            if m.id == memberA.id == memberB.id:
                return True
            elif m.id == memberA.id:
                foundA = True
            elif m.id == memberB.id:
                foundB = True
...
class NumberInputInterface(discord.ui.View):
    ...
    async def submit(self, interaction: discord.Interaction, button: discord.ui.Button):
        if self.post_func:
            await self.post_func(self.result)
...
class DateInputInterface(discord.ui.View):
    ...
    async def setupMonths(self):
        ...
        for i, month in enumerate(months.values(), 1):
            self.add_item(
                discord.ui.Button(
                    label=month, style=discord.ButtonStyle.blurple if self.result.month == i else discord.ButtonStyle.gray, custom_id=month
                )
            )
    ...
    async def selectMonth(self, comeFromSetup=False):
        ...
        try:
            interaction: discord.Interaction = await self.bot.wait_for('interaction', check=self.interactionCheck, timeout=300)
        except asyncio.TimeoutError:
    ...
    async def setupDays(self):
        ...
        for i in range(1, 24):  # The rest will be handled within the actual method
            self.add_item(
                discord.ui.Button(label=i, style=discord.ButtonStyle.blurple if self.result.day == i else discord.ButtonStyle.gray, custom_id=str(i))
            )
    ...
    async def selectDay(self):
        ...
        while True:
            ...
            if interaction.data['custom_id'] == 'switch':
                ...
                self.add_item(self.backButton)
                if firstHalf:
                    bounds = (24, daysPerMonth[self.result.month] + 1)
                else:
                    bounds = (1, 24)
                for i in range(*bounds):
                    self.add_item(
                        discord.ui.Button(
                            label=i, style=discord.ButtonStyle.blurple if self.result.day == i else discord.ButtonStyle.gray, custom_id=i
                        )
                    )
                self.add_item(discord.ui.Button(label='Rest of month', custom_id='switch'))
    ...
    async def confirmation(self):
        ...
        try:
            result: discord.Interaction = await self.bot.wait_for('interaction', check=self.interactionCheck, timeout=300)
        except asyncio.TimeoutError:
            return self.task.cancel()
        ...
        if result.data['custom_id'] == 'back':
            ...
            try:
                if self.result < datetime.datetime.now(tz=datetime.timezone.utc):
                    self.result.replace(year=self.result.year + 1)
            except:
                traceback.print_exc()
            if self.post_function:
                await self.post_function(self.result)
            await result.response.edit_message(
                content=f'`{self.result:%B %d}`', embed=None, view=SuccessView('Press "confirm" on the original embed to complete setup')
            )
...
class BirthdayHomepageView(discord.ui.View):
    ...
    async def createEmbed(self):
        ...
        if not self.user:
            self.user = await utility.get_user(self.interaction.user)
        ...
        if not await self.cyber.privacyEnabledChecker(self.interaction.user, 'default', 'birthdayModule'):
            ...
            if not self.interaction.response.is_done():
                return await self.interaction.response.send_message(embed=embed, view=self)
            else:
                return await self.interaction.followup.edit_message(embed=embed, view=self)
        embed.add_field(
            name='Your Birthday',
            value='Not configured'
            if not bday
            else 'Hidden'
            if self.interaction.guild and not await self.cyber.privacyVisibilityChecker(self.interaction.user, 'birthdayModule', 'birthdayDay')
        ...
        embed.add_field(
            ...
            if self.interaction.guild and not await self.cyber.privacyVisibilityChecker(self.interaction.user, 'birthdayModule', 'age')
            else age,
        )
        if len(wishlist) > 0:
            embed.add_field(
                name='Your Wishlist',
                value='Hidden'
                if self.interaction.guild and not await self.cyber.privacyVisibilityChecker(self.interaction.user, 'birthdayModule', 'wishlist')
                else f'{len(wishlist)} items',
            )
    ...
    async def finishEmbed(self, embed: discord.Embed):
        ...
        bday, age, wishlist = self.user.get('birthday'), self.user.get('age'), self.user.get('wishlist', [])
        memberIDs = (
            set([m.id for m in self.interaction.guild.members]) if self.interaction.guild else ()
        )  # June 2021 (v0.2.27): Changed from list to set to improve performance // Holds list of member IDs for the current server
        for user in self.birthdays.bot.users:
            try:
                ...
                if user.id in memberIDs:
                    currentServer.append({'data': user, 'bday': userBirthday})
                elif mutualServerMemberToMember(self.birthdays, self.interaction.user, user):
                    if (userBirthday - discord.utils.utcnow()).days < 8:
                        weekBirthday.append({'data': user, 'bday': userBirthday})
                    else:
                        disguardSuggest.append({'data': user, 'bday': userBirthday})
            except (AttributeError, TypeError, KeyError):
                pass
        currentServer.sort(key=lambda m: m.get('bday'))
        weekBirthday.sort(key=lambda m: m.get('bday'))
        disguardSuggest.sort(
            key=lambda m: len(mutualServersMemberToMember(self.birthdays, self.interaction.user, m['data'])), reverse=True
        )  # Servers the author and target share
        ...
        def fillBirthdayList(list, maxEntries):
            return [
                f"{qlfc}\\▪️ **{m['data'].display_name if firstNine.count(m['data'].display_name) == 1 else m['data']}** • {m['bday']:%a %b %d} • <t:{round(m['bday'].timestamp())}:R>"
                for m in list[:maxEntries]
            ]
        ...
        for item in [self.birthdayButton, self.ageButton, self.wishlistButton]:
            self.add_item(item)
        return embed
    ...
    class editBirthday(discord.ui.Button):
        ...
        async def callback(self, interaction: discord.Interaction):
            ...
            if await view.cyber.privacyEnabledChecker(interaction.user, 'birthdayModule', 'birthdayDay'):
                ...
                await interaction.response.send_message(
                    'You have disabled the birthday feature of the birthday module. Review your settings: http://disguard.herokuapp.com/manage/profile',
                    ephemeral=True,
                )
    ...
    class editAge(discord.ui.Button):
        ...
        async def callback(self, interaction: discord.Interaction):
            ...
            if await view.cyber.privacyEnabledChecker(interaction.user, 'birthdayModule', 'age'):
                ...
                await interaction.response.send_message(
                    'You have disabled the age feature of the birthday module. Review your settings: http://disguard.herokuapp.com/manage/profile',
                    ephemeral=True,
                )
    ...
    class editWishlist(discord.ui.Button):
        ...
        async def callback(self, interaction: discord.Interaction):
            ...
            if await view.cyber.privacyEnabledChecker(interaction.user, 'birthdayModule', 'wishlist'):
                ...
                await interaction.response.send_message(
                    'You have disabled the wishlist feature of the birthday module. Review your settings: http://disguard.herokuapp.com/manage/profile',
                    ephemeral=True,
                )
...
class GuestBirthdayView(discord.ui.View):
    ...
    def __init__(self, birthdays: Birthdays, interaction: discord.Interaction, target: discord.User):
        ...
        if interaction.user == target:
            self.add_item(self.OverviewButton(birthdays))
    ...
    async def createEmbed(self):
        ...
        embed = discord.Embed(
            title=f"🍰 {self.target.display_name}'s Birthday Page",
        ...
        embed.add_field(
            ...
            if not await cyber.privacyVisibilityChecker(self.target, 'birthdayModule', 'birthdayDay')
            else f'{bday:%a %b %d}\n(<t:{round(bday.timestamp())}:R>)',
        )
        embed.add_field(
            name='Age',
    ...
    class MessageButton(discord.ui.Button):
        ...
        async def callback(self, interaction: discord.Interaction):
            view: GuestBirthdayView = self.view
            if interaction.user == view.target:
                await interaction.response.send_message("You can't write a birthday message to yourself!", ephemeral=True)
            else:
...
class AgeView(discord.ui.View):
    ...
    async def createEmbed(self):
        ...
        instructions = (
            'Since your age visibility is set to private, use the virtual keyboard (edit privately button) to enter your age'
            if self.ageHidden
            else 'Type your desired age'
            if not self.newAge
            else f'{self.author.display_name} | Update your age to **{self.newAge}**?'
            if 13 <= self.newAge <= 105
            else f'⚠ | **{self.newAge}** falls outside the age range of 13 to 105, inclusive. Please type a new age.'
        )
        embed = discord.Embed(
            title='🕯 Birthday age setup',
            description=f'{ageModuleDescription if not self.currentAge else ""}{instructions}\n\nCurrent value: {"🔒 Hidden" if self.ageHidden else self.currentAge}',
            color=yellow[await utility.color_theme(self.interaction.guild)],
        )
    ...
    async def privateInterface(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        for child in self.children:
            child.disabled = True  # Disable all buttons for proper control flow
        ...
        if result.data['custom_id'] == 'cancel':
            await interaction.message.edit(view=self)  # Enable buttons if cancelling virtual keybaord operation
    ...
    async def confirmation(self):
        while not self.birthdays.bot.is_closed() and not self.finishedSetup:
            ...
            def interactionCheck(i: discord.Interaction):  # TODO: needs more verification
                if i.data['custom_id'] == 'cancel':
                    self.usedPrivateInterface = False
                return (
                    i.data['custom_id'] in ('submit', 'cancel', 'confirmSetup', 'cancelSetup')
                    and i.user == self.author
                    and i.message.id == self.message.id
                )
            ...
            if not self.usedPrivateInterface and not self.ageHidden:
                ...
                if not self.usedPrivateInterface:
                    self.newAge = calculateAges(result)[0]  # If private interface was used, submitValue will store the value
                try:
            ...
            if self.newAge:
                if self.newAge < 13 or self.newAge > 110:
                    self.embed.description = (
                        f'⚠ | **{self.newAge}** falls outside the valid age range of 13 to 110, inclusive. Please type another age.'
                    )
                ...
                self.embed.description = f'{self.birthdays.emojis["alert"]} | **{"the value you entered" if self.usedPrivateInterface else self.newAge}** isn\'t an age. You may type a new age or cancel the setup.'
            try:
                if self.interaction.message:
                    await self.intermediate_message.edit(embed=self.embed, view=self)
                elif self.autoDetected:
                    break
                if self.usedPrivateInterface:
                    break
    ...
    async def saveChanges(self):
        ...
        if self.newAge == self.currentAge:
            return await self.interaction.message.delete()
        ...
        if not self.autoDetected:
            ...
            self.interaction.message.embeds[0].set_field_at(
                1, name='Your Age', value=f'**Age Successfully Updated**\n{"🔒 Hidden" if self.usedPrivateInterface else self.newAge}'
            )
...
class BirthdayView(discord.ui.View):
    ...
    async def createEmbed(self):
        ...
        instructions = (
            'Since your birthday visibility is set to private, please use the virtual keyboard (edit privately button) to enter your birthday'
            if self.bdayHidden
            else 'Type your birthday or use the virtual keyboard (`edit privately` button) for your input. Disguard does not process your birth year, so just enter a month and a day.'
            if not self.newBday
            else f'{self.author.display_name} | Update your birthday to **{self.newBday}**?'
        )
        acceptableFormats = 'Inputs are not case sensitive. Examples of acceptable birthday formats:\nFebruary 28\nFeb 28\nFeb 28th\n2/28\n2-28\n02 28\nin two weeks\nin 5 days\nnext monday\ntomorrow'
        embed = discord.Embed(
            title='📆 Birthday date setup',
            description=f'{birthdayModuleDescription if not self.currentBday else ""}{instructions}{f"{NEWLINE}{NEWLINE}{acceptableFormats}" if not self.newBday else ""}\n\nCurrent value:  {"🔒 Hidden" if self.bdayHidden else self.currentBday.strftime("%B %d") if self.currentBday else "Not set"}',
            color=yellow[await utility.color_theme(self.interaction.guild)],
        )
        embed.set_author(name=self.author.display_name, icon_url=self.author.display_avatar.url)
        self.embed = embed
    ...
    async def confirmation(self):
        while not self.birthdays.bot.is_closed() and not self.finishedSetup:
            ...
            def interactionCheck(i: discord.Interaction):  # TODO: needs more verification
                if i.data.get('custom_id') == 'cancel':
                    self.usedPrivateInterface = False
                return (
                    i.data.get('custom_id') in ('submit', 'cancel', 'confirmSetup', 'cancelSetup')
                    and i.user == self.author
                    and i.channel == self.interaction.channel
                )
            ...
            if self.newBday:
                ...
                self.embed.description = f'{self.birthdays.emojis["alert"]} | Unable to parse a date from **{"the value you entered" if self.usedPrivateInterface else self.newBday}**. You may type a new date or cancel the setup.'
            try:
                if self.interaction.message:
                    await self.intermediate_message.edit(embed=self.embed, view=self)
                elif self.autoDetected:
    ...
    async def saveChanges(self):
        ...
        self.embed.description = (
            f'✔ | Birthday successfully updated to {"(hidden value)" if self.usedPrivateInterface else self.newBday.strftime("%B %d")}'
        )
        bdayAnnounceChannel = (await utility.get_server(self.interaction.guild)).get('birthday', 0)
        if bdayAnnounceChannel > 0:
            bdayAnnounceText = f'Since birthday announcements are enabled for this server, your birthday will be announced to {self.birthdays.bot.get_channel(bdayAnnounceChannel).mention}.'
        else:
            bdayAnnounceText = f'Birthday announcements are not enabled for this server. Moderators may enable this feature [here](http://disguard.herokuapp.com/manage/{self.interaction.guild.id}/server).'
...
class WishlistView(discord.ui.View):
    def __init__(
        ...
        birthdays: Birthdays,
        interaction: discord.Interaction,
        previousView: BirthdayHomepageView,
        wishlist_hidden: bool = False,
        mode: str = 'home',
        cameFromSaved=[],
    ):
        super().__init__()
    ...
    async def createEmbed(self):
        self.wishlist = (
            (await utility.get_user(self.interaction.user)).get('wishlist', [])
            if not self.wishlistHidden and not self.cameFromSaved
            else self.cameFromSaved
            if self.cameFromSaved
            else []
        )
        wishlistDisplay = ['You have set your wishlist to private'] if self.wishlistHidden else self.wishlist
        embed = discord.Embed(
            title='📝 Wishlist home',
            description=f'**{"YOUR WISH LIST":–^50}**\n{NEWLINE.join([f"• {w}" for w in wishlistDisplay]) if wishlistDisplay else "Empty"}',
            color=yellow[await utility.color_theme(self.interaction.guild)],
        )
        embed.set_author(name=self.interaction.user.display_name, icon_url=self.interaction.user.display_avatar.url)
        if self.cameFromSaved:
            embed.set_footer(text='Wishlist changes successfully saved')
        return embed
    ...
    async def close(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            await interaction.response.pong()
            await interaction.message.delete()
            if not self.previousView or isinstance(self.previousView, WishlistView):
                return
    ...
    async def wishlistEditPreview(self, interaction: discord.Interaction, add=True):
        ...
        if interaction.message:
            ...
            await interaction.response.send_message(embed=embed, view=new_view, ephemeral=self.wishlistHidden)
...
class WishlistEditView(discord.ui.View):
    ...
    class saveButton(discord.ui.Button):
        ...
        async def callback(self, interaction: discord.Interaction):
            view: WishlistEditView = self.view
            if not view.add:
                self.tempWishlist = [w for w in self.tempWishlist if not view.toModify.get(w[:16])]
    ...
    async def editWishlist(self):
        def addCheck(m: discord.Message):
            return m.author == self.interaction.user and m.channel == self.interaction.channel

        while not self.birthdays.bot.is_closed():
            try:
                message: discord.Message = await self.birthdays.bot.wait_for('message', check=addCheck, timeout=300)
            except asyncio.TimeoutError:
                view = WishlistView(self.birthdays, self.interaction, self.previousView)
                embed = await view.createEmbed()
            ...
            if message.content:
                ...
                if self.add:
                    for word in words:
                        self.toModify[word[:16]] = word  # O(n)
    ...
    async def refreshDisplay(self, respond_to: discord.Interaction | bool = False):
        def formatWishlistEntry(s: str):
            return (
                f'**+ {s}**'
                if self.add and self.toModify.get(s[:16])
                else f'~~{s}~~'
                if not self.add and self.toModify.get(s[:16])
                else f'• {s}'
                if self.add
                else s
            )
        ...
        if self.tempWishlist and not (all([w == self.toModify.get(w[:16]) for w in self.tempWishlist]) and not self.add):
            self.buttonClear.disabled = False
        else:
            self.buttonClear.disabled = True
        if respond_to:
            await respond_to.response.edit_message(embed=embed, view=self)
        else:
...
class UpcomingBirthdaysView(discord.ui.View):
    def __init__(
        ...
        self.embed = embed
        self.currentView = (
            (self.currentServer[0][:8] if self.currentServer else [])
            + (self.disguardSuggest[0][:8] if self.disguardSuggest else [])
            + (self.weekBirthday[0][:8] if self.weekBirthday else [])
        )
        self.currentPage = 0
        self.finalPage = 0
    ...
    def fillBirthdayList(self, list, page=0, entries=25):
        return [
            f"{qlfc}\\▪️ **{m['data'].display_name if self.namesOnly.count(m['data'].display_name) == 1 else m['data']}** • {m['bday']:%a %b %d} • <t:{round(m['bday'].timestamp())}:R>"
            for m in (list[page][:entries] if list else [])
        ]

    def paginate(self, data):
        for i in range(0, len(data), 25):
            yield data[i : i + 25]  # 25 entries per page
    ...
    class nextPage(discord.ui.Button):
        ...
        async def callback(self, interaction: discord.Interaction):
            view: UpcomingBirthdaysView = self.view
            if view.currentPage == view.finalPage:
                return await interaction.response.pong()
            view.currentPage += 1
            if view.currentPage == view.finalPage:
                self.disabled = True  # teething issues
    ...
    class selectMemberDropdown(discord.ui.Select):
        def __init__(self, population=[], custom_id=None):
            if custom_id:
                super().__init__(placeholder=f'Select a member ({len(population)} result{"" if len(population) == 1 else "s"})', custom_id=custom_id)
            else:
                super().__init__(placeholder=f'Select a member ({len(population)} result{"" if len(population) == 1 else "s"})')
        ...
        async def callback(self, interaction: discord.Interaction):
            view: UpcomingBirthdaysView = self.view
            newView = ComposeMessageView(
                view.birthdays, self.userDict[int(self.values[0])], view.author, view.interaction.channel, view.interaction.message, view
            )
            embed = await newView.createEmbed()
            await interaction.response.edit_message(embed=embed, view=newView)
            await newView.writeMessagePrompt()
    ...
    class searchMembersButton(discord.ui.Button):
        ...
        async def callback(self, interaction: discord.Interaction):
            ...
            view.embed.description = (
                'Send a message to search members in across our mutual servers, then select your desired result from the dropdown'
            )
            select = view.memberDropdown

            def messageCheck(m: discord.Message):
                return m.author == view.author and m.channel == view.interaction.channel

            def selectCheck(i: discord.Interaction):
                return i.user == view.author and i.data['custom_id'] == str(view.interaction.message.id)

            async def getBirthday(u: discord.User):
                return (await utility.get_user(u)).get('birthday')
            ...
            while not view.bot.is_closed():
                ...
                for f in pending:
                    f.cancel()
                if type(result) is discord.Interaction:
                    break
...
class ComposeMessageView(discord.ui.View):
    ...
    async def createEmbed(self):
        ...
        self.embed = discord.Embed(title='Compose birthday message', description=intro)
        return self.embed

    async def writeMessagePrompt(self):
        if not isinstance(self.channel, discord.DMChannel):
            return  # ensures we only proceed if in DMs

        def messageCheck(m: discord.Message):
            return m.author == self.author and type(m.channel) is not discord.TextChannel

        satisfactoryMessage = False
        self.stage = 0
        while not satisfactoryMessage:
            try:
                self.msgInBottle: discord.Message = await self.birthdays.bot.wait_for('message', check=messageCheck, timeout=300)
            except asyncio.TimeoutError:
                prev: UpcomingBirthdaysView = self.previousView
                await prev.createEmbed()
                return await prev.loadHomepage()
            satisfactoryMessage = (
                not re.search('.*discord.gg/.*', self.msgInBottle.content)
                and not re.search('.*htt(p|ps)://.*', self.msgInBottle.content)
                and len(self.msgInBottle.content) < 1024
            )
    ...
    async def selectDestinationsPrompt(self):
        ...
        if not self.target.dm_channel:
            try:
                ...
                dmChannel = None
        dropdown.add_option(
            label=f"{self.target.display_name}'s DMs", value=self.target.dm_channel.id, description='Recommended' if dmChannel else 'Unable to DM'
        )
        for server in mutualServers:
            ...
            dropdown.add_option(
                label=server.name,
                value=server.id,
                description=f'#{birthdayChannel.display_name}' if birthdayChannel else 'No announcement channel configured',
            )
        ...
        try:
            await self.birthdays.bot.wait_for('interaction', check=interactionCheck, timeout=300)
        except asyncio.TimeoutError:
            prev: UpcomingBirthdaysView = self.previousView
            await prev.createEmbed()
            return await prev.loadHomepage()
    ...
    async def confirmationPrompt(self, dropdown: discord.ui.Select):
        ...
        destinations = [f'• {self.birthdays.bot.get_channel(int(dropdown.values[0]))}'] + [
            f'• {self.birthdays.bot.get_guild(int(v))}' for v in (dropdown.values[1:] if len(dropdown.values) > 1 else [])
        ]
        ...
        self.embed.description += 'If this message is not appropriate for Discord, the recipient(s) may flag it for further action by server moderators or the developer of Disguard.\n\nIf this all looks good, press the green button.'
        self.clear_items()
        self.add_item(self.buttonBack)
        ...
        def finalCheck(i: discord.Interaction):
            return i.data['custom_id'] in ('restart', 'confirm') and i.user == self.author and type(i.channel) is not discord.TextChannel

        try:
            result: discord.Interaction = await self.birthdays.bot.wait_for('interaction', check=finalCheck, timeout=300)
        except asyncio.TimeoutError:
            prev: UpcomingBirthdaysView = self.previousView
            await prev.createEmbed()
            return await prev.loadHomepage()
        ...
        await database.SetBirthdayMessage(
            self.target, self.msgInBottle, self.author, serverDestinations
        )  # TODO: figure out how this relates to members receiving DMs, maybe enable by default and the dropdown can be to select additional servers
    ...
    class Dropdown(discord.ui.Select):
        def __init__(self, entries):
            ...
            for i, entry in enumerate(entries):
                self.add_option(label=entry, value=i)
