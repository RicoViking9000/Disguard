...
class LogModule(object):
    ...
    def __init__(self, name, description, embed=None, audit=None, enabled=True, summarize=0, channelID=None, color=None, library=None, thumbnail=None, author=None, emojiContext=None, hoverLinks=None, timestamp = None, botLogging=None, flashText = None, tts=None):
        ...
        self.botLogging = 0 if name == 'message' else botLogging #To avoid spam | 0: Disabled, 1: Plaintext, 2: Embeds
...
async def VerifyServer(s: discord.Guild, b: commands.Bot, newOnly=False, full=False):
    ...
    if not serv or full: 
        ...
        "cyberlog": {
            ...
            "message": vars(LogModule("message", "Send logs when a message is edited or deleted")) if log is None or log.get('message') is None else vars(LogModule("message", "Send logs when a message is edited or deleted").update(log.get('message'))),
            "doorguard": vars(LogModule("doorguard", "Send logs when a member joins or leaves server")) if log is None or log.get('doorguard') is None else vars(LogModule("doorguard", "Send logs when a member joins or leaves server").update(log.get('doorguard'))),
            "channel": vars(LogModule("channel", "Send logs when channel is created, edited, or deleted")) if log is None or log.get('channel') is None else vars(LogModule("channel", "Send logs when channel is created, edited, or deleted").update(log.get('channel'))),
            ...
            "role": vars(LogModule("role", "Send logs when a role is created, edited, or deleted")) if log is None or log.get('role') is None else vars(LogModule("role", "Send logs when a role is created, edited, or deleted").update(log.get('role'))),
            "emoji": vars(LogModule("emoji", "Send logs when emoji is created, edited, or deleted")) if log is None or log.get('emoji') is None else vars(LogModule("emoji", "Send logs when emoji is created, edited, or deleted").update(log.get('emoji'))),
            "server": vars(LogModule("server", "Send logs when server is updated, such as thumbnail")) if log is None or log.get('server') is None else vars(LogModule("server", "Send logs when server is updated, such as thumbnail").update(log.get('server'))),
            "voice": vars(LogModule('voice', "Send logs when members' voice chat attributes change")) if log is None or log.get('voice') is None else vars(LogModule('voice', "Send logs when members' voice chat attributes change").update(log.get('voice'))),
            "misc": vars(LogModule('misc', "Logging for various bonus features that don't fit into an above category (currently only ghost reaction logging)")) if log is None or log.get('misc') is None else vars(LogModule('misc', "Logging for various bonus features that don't fit into an above category").update(log.get('misc')))
            }}}, upsert=True)
            # 'modules': [
