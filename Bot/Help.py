...
class Help(commands.Cog):
    ...
    async def help_autocomplete(self, interaction: discord.Interaction, argument: str):
        return [discord.app_commands.Choice(name=command.name, value=command.name) for command in self.bot.commands if argument.lower() in command.name or not argument][:25]
