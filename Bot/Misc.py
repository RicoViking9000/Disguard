...
class Misc(commands.Cog):
        ...
        for match in matches:
            ...
            if result.channel.is_nsfw() and not message.channel.is_nsfw():
                return await message.channel.send(
                    f'[Jump Context] {self.emojis["alert"]} | This message links to a NSFW channel, so I can\'t reshare the message'
                )
            if result.embeds and not message.author.bot:
                if not result.embeds[0].footer.text:
                    result.embeds[0].set_footer(
                        text=f'{(result.created_at + datetime.timedelta(hours=await utility.time_zone(message.guild))):%b %d, %Y - %I:%M %p} {await utility.name_zone(message.guild)}'
                    )
                if not result.embeds[0].author.name:
                    result.embeds[0].set_author(name=result.author.display_name, icon_url=result.author.display_avatar.url)
                ...
                if len(result.attachments) > 0 and result.attachments[0].height is not None:
                    try:
                        embed.set_image(url=result.attachments[0].url)
                    except:
                        pass
    ...
    async def pause(self, ctx: commands.Context, module: str, seconds: typing.Optional[int] = 0):
        ...
        if seconds == 0:
            return  # If the duration is infinite, we don't wait
    ...
    class AttributeHistoryView(discord.ui.View):
        ...
        class SelectAnAttributeDropdown(discord.ui.Select):
            ...
            async def callback(self, interaction: discord.Interaction):
                try:
                    view: Misc.AttributeHistoryView = self.view
                    if view.attribute:
                        self.options[view.attributes.index(view.attribute)].default = False
        ...
        class SelectAnAvatarDropdown(discord.ui.Select):
            def __init__(self, data: list[str]):
                ...
                super().__init__(
                    ...
                    options=[discord.SelectOption(label='Clear image', value='clear')]
                    + [discord.SelectOption(label=f'Avatar {i+1}', value=str(i)) for i, _ in enumerate(data)],
                )
                self.data = data
        ...
        class SelectACustomStatusDropdown(discord.ui.Select):
            def __init__(self, data: list[dict[str, str]]):
                super().__init__(
                    ...
                    options=[discord.SelectOption(label='Clear image', value='clear')]
                    + [discord.SelectOption(label=f'Status emoji {i+1}', value=str(i)) for i, _ in enumerate(data)],
                )
                self.data = data
                self.selected_display = 0
        ...
        class LinkToAllSettings(discord.ui.Button):
            def __init__(self):
                super().__init__(
                    style=discord.ButtonStyle.link, label='Manage all privacy settings', url='https://disguard.heroku.app/manage/profile'
                )
        ...
        async def setup(self):
            ...
            if self.member and self.attribute:
                ...
                self.embed.description = 'Select a member and history attribute to view history data'
                return -1
        ...
        def update_page_buttons(self):
            ...
            if self.current_page == len(self.page_count) - 1:
                self.next_button.disabled = True
            elif self.current_page != len(self.page_count) - 1 and self.next_button.disabled:
        ...
        async def update_display(self):
            ...
            if self.attribute in ('username', 'displayname'):
                self.embed.set_thumbnail(url=self.member.display_avatar.url)
            ...
            for i, entry in enumerate(data):
                if i > 0:
                    ...
                    for j in range(len(times) - 1, -1, -1):
                        if times[j] != 0:
                            distanceDisplay.append(f'{times[j]} {units[j]}{"s" if times[j] != 1 else ""}')
                    if len(distanceDisplay) == 0:
                        distanceDisplay = ['0 seconds']
                prior = entry
    ...
    async def say(self, ctx: commands.Context, member: discord.Member = None, channel: discord.TextChannel = None, *, message: str = 'Hello World'):
        ...
        webhook = await channel.create_webhook(
            name='automationSayCommand',
            avatar=await member.display_avatar.with_static_format('png').read(),
            reason=f'Initiated by {ctx.author.display_name} to imitate {member.display_name} by saying "{message}"',
        )
